...
## Зависимости     
:fuelpump: Все зависимости прописаны в файле `requirements.txt`
...
## Настройки      
:ticket: ПЕРЕД ЗАПУСКОМ обязательно надо соединить COM порты c компьютером и задать настройки в файле
...
## ТЕСТЫ:     
:fuelpump: Все тесты направлены на тестирование модуля транзита по схеме:
...
-----------
### PyTest    
:monorail: Были сделаны ради быстрой проверки модуля - работает или нет
...
### Запуск в ручную     
:bullettrain_side: Так же в корневой директории содержаться классы для проведения тестовых прогонов учитывая различные сценарии
#### Транзит c COM порта на TCP порт     
:train2: Тесты на транзит с просушкой одного порта TCP и всех  
...
:train2: Тесты на транзит с просушкой одного порта COM и всех     
...
:train:   В качестве полного теста транзита и возможности эмуляции обмена со счетчиком командами используется тесты эмуляции обмена        
...
:ambulance:  Отваливание сокета по таймауту    
