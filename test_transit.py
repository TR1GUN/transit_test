...
def test_transit_All_Ports_TCPtoCOM(data, COM):

    from Transit_All_Ports_TCP_to_COM import AllTCPtoCOM
    from time import sleep
    TransitTCPtoCOM = AllTCPtoCOM(data=data).Setup(COM=COM)
    # sleep(5)

@pytest.mark.parametrize("data, COM", parametrize(parametrize_TCPtoCOM))
def test_transit_All_Ports_COMtoTCP(data, COM):

    from Transit_All_Ports_COM_to_TCP import AllCOMtoTCP
    from time import sleep
    TransitCOMtoTCP = AllCOMtoTCP(data=data).Setup(COM=COM)
    # sleep(5)
